...
CMD ([ -f /usr/share/Inedo/SharedConfig/ProGet.config ] || echo '<?xml version="1.0" encoding="utf-8"?><InedoAppConfig><ConnectionString Type="'"$PROGET_DB_TYPE"'">'"$PROGET_DATABASE"'</ConnectionString><WebServer Enabled="true" Urls="http://*:3067/"/></InedoAppConfig>' > /usr/share/Inedo/SharedConfig/ProGet.config) \
